...
Open `http://localhost:4173`. The browser accepts only a user-provided registered Pollinations `sk_` Secret Key. The key is kept in `sessionStorage` for this browser tab only; use a temporary or scoped key because browser code can access it. No key is committed, logged, placed in an image URL, or sent in a request body. This repository embeds no secret and does not provision OAuth.
...
The browser calls `https://gen.pollinations.ai/v1/chat/completions` and `https://gen.pollinations.ai/image/{encoded-prompt}` with the entered `sk_` key in an `Authorization: Bearer` header. Settings includes an allowlisted text-model selector: GPT-5 Nano (`openai-fast`, default), GPT-5.4 Nano (`openai`), Claude Fast, Gemini Fast, DeepSeek, and Mistral Small 3.2. The selected model is saved as a local preference for new or failed discoveries; already-cached recipes remain pair-keyed and are reused. Model output is bounded, parsed as JSON, validated, and rendered as text—not HTML. Image responses are temporary object URLs and are revoked when replaced or the page closes.
